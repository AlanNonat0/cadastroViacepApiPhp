...
class ViaCepDao
{
    ...
    public static function register(array $request) : string
    {
        ...
        $request['numb'] = $request['numb'] == '0'? 'S/N': $request['numb']; 

        if ($error) {
            return "connecterror";
        } else {
            $sql = "INSERT INTO address (`name`, `cep`, `public_place`, `numb`, `district`, `city`, `state`, `complement`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            $stmt->bindValue(8, $request['complement']);
            $stmt->execute();
            return "success";
        }
